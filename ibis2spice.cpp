...
std::string ibis2spice::_gen_ngspice_model_by_model_output(ibis::component& component, ibis::model model, std::string type)
{
    ...
    sprintf(buf, ".SUBCKT %s_%s_OUTPUT IN OUT VCC VSS freq=800Meg duty=0.5\n", _make_upper(model.name).c_str(), _make_upper(type).c_str());
    subckt = subckt + buf;
    subckt = subckt + "\n";
...
std::string ibis2spice::_gen_ngspice_model_by_model_input(ibis::component& component, ibis::model model, std::string type)
{
    ...
    rising = _read_two_col(model.rising_waveforms[0].values, type);
    falling = _read_two_col(model.rising_waveforms[0].values, type);
    ...
    if (!model.add_submodels.empty())
    {
        ibis::model submodel;
        if (_ibs.get_model_by_name(model.add_submodels[0].submodel_name, submodel))
        {
            subckt = subckt + _gen_ngspice_model_by_model_input_submodel(component, submodel, type);
        }
    }
    subckt = subckt + ".ENDS\n";
    subckt = subckt + "\n";
...
std::string ibis2spice::_gen_ngspice_model_by_model_input_submodel(ibis::component& component, ibis::model model, std::string type)
{
    std::string subckt;
